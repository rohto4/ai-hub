...
7. `title + summary_200` から隣接分野タグを 1〜2 件抽出し、`thumbnail_bg_theme` を決定する

## 5. publish / ranking の要点

1. `hourly-publish` は L2 の publish candidate を `public_articles` へ upsert
2. `public_article_tags` / `public_article_adjacent_tags` / `public_article_sources` を同期
3. `hourly-compute-ranks` は `public_articles` を 1 回読み込み、4 window を並列計算して upsert
