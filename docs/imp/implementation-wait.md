...
判断待ち:
- `paper` 専用タグマスタを別途持つか
- `paper` 判定時に通常タグ系から即切り替えるロジックを入れるか
- `tags_master` に将来的な tag group / paper-only 属性を持たせるか
- `paper` 専用タグを公開面でどこまで見せるか

次の確認:
- 719 / 720 の原因分析は確認済み
- 直近は先に、主タグ / 新規立項タグ / カテゴリ / 周辺分野タグ / 公開導線の整理を進める
- `paper` 専用タグ候補の初期セットと切替ロジックは `implementation-plan` 上の後続タスクとして保留する
...
現状:
- 主タグ系は `tags_master` / `tag_aliases` / `tag_keywords` / `tag_candidate_pool` に分かれている
- 周辺分野タグ系は `adjacent_tags_master` / `adjacent_tag_keywords` に分かれている
- 記事への紐付けも L2/L4 で主タグ系と周辺分野タグ系が別テーブルになっている

論点:
- 今後タグ種別が増える場合、テーブルを増やし続けるか
- 共通カラムが多いなら、単一系テーブル + `tag_type` でまとめる方が筋が良いか
- 一方で、公開導線・重み・マッチロジック・運用責務が異なるなら分離維持の方が安全か

判断待ち:
- `tag_type` を導入した共通タグテーブルへ将来的に寄せるか
- それとも主タグ / 周辺分野タグ / 論文専用タグを別マスタのまま保つか
- 移行コストに見合うほど、運用・実装メリットがあるか

直近方針:
- 今は破壊的変更を避け、現行テーブル構成のまま主タグ整理と公開導線を先に進める
- 再編判断は後続タスクとして `implementation-plan` に残す
...
現状:
- 周辺分野タグは `thumbnail_bg_theme` と文脈補助のために導入済み
- まだ専用 UI / ページは未実装

直近方針:
- 今は通常タグと同様にクリック可能な導線で扱えばよい
- Web 完成の必須条件には置かない

将来案:
- 特定タグを見たとき、そのタグがどの分野で活用されているかを視覚的に見るマッピングページを作る
- 周辺分野タグはそのマッピング導線の基盤として使う
