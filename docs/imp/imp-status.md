...
- Home 公開面は開きっぱなし時の自動更新・自動受信を持たない方針に切り替え、クリック起点以外の復帰挙動と PWA/service worker の既定有効化を外した
- `ui-memo.md` 14/15/16/19/20 を反映し、記事カード footer は緑枠内幅を基準に 4 列+隙間=100% の比率指定へ変更、関連トピックは外枠なし+左縦線 `#c77719`、サムネイル上の `LANG` 表示を削除し、カード shadow + hover 浮き、上段白エリア固定高さも追加した。短文モードはカード全高 215px を基準に再配分し、白エリア 168px + footer 35px + 外余白 4px 系へ調整した
- 短文/長文でずれていた白エリア外周と footer 下余白も統一し、カード外周マージンは共通 4px に揃えた。長文モード全高は 296px に再調整した
- デスクトップ向けにサイト全体のカスタムカーソルを追加し、通常カーソルの代わりに発光した「ロボット + 左上紙飛行機」カーソルを追従表示するようにした
- カスタムカーソルの SVG 本体を `CustomCursorArt.tsx` に分離し、紙飛行機 / アンテナ / 頭 / 耳 / 目を個別編集しやすい形へ整理した
...
33. Home の `return focus` 復元を外し、PWA install / service worker を env opt-in 化して既定では無効化した
34. Home 記事カードの footer レイアウトを緑枠内基準の比率指定へ寄せ、関連トピック・LANG 表示・shadow/hover・上段白エリア固定高さを `ui-memo.md` に合わせて調整した
