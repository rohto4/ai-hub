...
| :------------------------- | :----------------------------------------------------------------------------------------------- |
| `docs/guide/.agent/skills/`           | 20以上の専門化されたAIペルソナ（フロントエンド、バックエンド、アーキテクチャ、品質管理等）を設定するプロンプト集 |
| `docs/guide/.agent/workflows/`        | 16の多段ステップSOPとスラッシュコマンド（例: `/deep-research` 等）を格納するエージェント起動領域      |
| `docs/guide/claude/`, `docs/guide/gemini/`, `docs/guide/codex/` | 各プラットフォーム（LLM）特有の制約や初期起動インプットを束ねたエントリポイント設定ファイル群             |

---

## 📌 プロジェクト固有ガイド

- [PROJECT.md](/G:/devwork/ai-summary/docs/guide/PROJECT.md)
  - AI Trend Hub 固有の恒久ルールをまとめたガイド
  - `docs/imp/` に置くべき進捗情報とは分離して管理する
