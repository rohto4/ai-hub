...
1. **認証（Authentication）の検証**
   - エンドポイントの先頭で Firebase Admin SDK の `auth.verifyIdToken()` によるトークン検証が行われているか確認する。
   - Firebase Admin SDK のサービスアカウントキーがフロントエンド（Client Components）に漏洩していないか全文検索で確認する。
...
2. **認可（Authorization / IDOR対策）の検証**
   ...
   - Neon のマイグレーション SQL において、該当テーブルの Row Level Security (RLS) が有効であり、パーミッションが設定されているか確認する（`docs/spec/04-data-model-and-sql.md` の RLS 方針に準拠）。
