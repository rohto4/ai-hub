 ...
 *   npx tsx scripts/run-enrich-worker.ts --queue-type paper --limit 20
...
const queueTypeArg = process.argv.indexOf('--queue-type')
...
const queueTypeRaw = queueTypeArg !== -1 ? process.argv[queueTypeArg + 1] : null
const queueType =
  queueTypeRaw === 'paper' || queueTypeRaw === 'all' || queueTypeRaw === 'non-paper'
    ? queueTypeRaw
    : sourceKey
      ? 'all'
      : 'non-paper'
...
runDailyEnrich({ limit, sourceKey, queueType, summaryBatchSize, maxSummaryBatches })
  .then((result) => {
