...
const inputPath = path.join(root, 'artifact/gemini-cli-enrich-backlog-1500/inputs/ai-enrich-inputs-part-006.json');
const tagMasterPath = path.join(root, 'artifact/gemini-cli-enrich-backlog-1500/prompts/tag-master.json');
const outputPath = path.join(root, 'artifact/gemini-cli-enrich-backlog-1500/outputs/ai-enrich-outputs-part-006.json');
...
async function buildItem(item, maps, index, total) {
  ...
  if ((index + 1) % 20 === 0 || index === 0 || index + 1 === total) {
    console.log(`[part006] ${index + 1}/${total} raw=${item.rawArticleId}`);
  }
